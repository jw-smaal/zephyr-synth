			     ...
			     bool use_log)
{
	...
	switch(padsr->state) {
		case ATTACK:
			...
			padsr->current_gain = (q15_t)(((padsr->initial_lifetime - padsr->lifetime) * 32767U) / padsr->initial_lifetime);
			break;
		case DECAY: {
			/* Start at FULL_LEVEL go down to SUSTAIN level */
			uint32_t distance = 32767U - padsr->sustain_level;
			padsr->current_gain = padsr->sustain_level + (q15_t)(((uint64_t)distance * padsr->lifetime) / padsr->initial_lifetime);
			break;
		}
		case SUSTAIN:
		...
		case RELEASE:
			...
			padsr->current_gain = (q15_t)((padsr->lifetime * padsr->sustain_level) / padsr->initial_lifetime);
			break;
		case END:
			break;
	};
	...
	if (use_log) {
		...
		padsr->current_gain = (q15_t)((int32_t)padsr->current_gain * padsr->current_gain)>>15;
	}
